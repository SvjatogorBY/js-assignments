...
function extractNameFromTemplate(value) {
    return value.replace('Hello, ','').match(/.+(?=!)/).join();
}
...
function getRectangleString(width, height) {
    var horisontal = '─'.repeat(width-2);
    var vertical = '│' + ' '.repeat(width-2) + '│\n';

    return '┌'+horisontal+'┐\n'+
            vertical.repeat(height-2)+
            '└'+horisontal+'┘\n';
}
...
function replacer(match) {
    ...
    var replacer = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';

    return replacer[alphabet.search(match)];
}
...
function getCardId(value) {
    return value.replace(/([\w]+)([^\w])/,cardReplacer);
}

function cardReplacer(match,p1,p2,offset,string)
{
    var suits='♣♦♥♠';
    var ranks='A23456789TJQK';
    p1 = p1==='10' ? 'T' : p1;

    return ranks.search(p1) + suits.search(p2) * 13;
}
